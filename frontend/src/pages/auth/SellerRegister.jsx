...
export default function SellerRegister() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccessMessage("Seller registration successful! You can now log in.");
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center w-full py-16 px-2 md:px-12">
        <div className="w-full max-w-lg bg-white/80 backdrop-blur-2xl rounded-3xl shadow-2xl p-10 flex flex-col items-center border border-green-100 relative">
          <h2 className="text-4xl font-extrabold mb-2 text-green-700 tracking-tight drop-shadow text-center">Seller Registration</h2>
          <p className="mb-6 text-gray-600 text-lg font-medium text-center">Create your seller account</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 border border-green-200 rounded-xl focus:ring-2 focus:ring-green-400 focus:border-green-400 transition outline-none shadow-sm bg-green-50 text-base"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-green-400 cursor-pointer" onClick={() => setShowConfirm(v => !v)}>
          ...
          <div className="mt-6 text-center space-y-2">
            <p className="text-sm text-gray-600">
              Already have a seller account? <Link to="/seller-login" className="font-semibold text-green-600 hover:underline">Login Now</Link>
            </p>
            <p className="text-sm text-gray-600">
              Want to register as user? <Link to="/user-register" className="font-semibold text-blue-600 hover:underline">User Registration</Link>
            </p>
          </div>
        </div>
      </div>
